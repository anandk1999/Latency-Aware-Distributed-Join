...
class KafkaLatencyAwareJoinOptimizer:
    ...
    def choose_join_strategy(self, left_size, right_size, network_latency, BROADCAST_THRESHOLD = 10*1024*1024, HIGH_LATENCY_THRESHOLD = 250):
        """
        ...
        if smaller_size <= BROADCAST_THRESHOLD:
            return 'broadcast'
        elif network_latency >= HIGH_LATENCY_THRESHOLD:
            return 'merge' #sort merge join
        else:
            return 'shuffle_hashs'
        ...
        # if avg_latency < 30:
        #     return 'shuffle'
        # elif min(left_size, right_size) < 1000:
        #     return 'broadcast'
        # else:
        #     return 'sort_merge'
    ...
    def distributed_join(self, left_df, right_df, join_key):
        ...
        left_size = left_df.explain(mode="cost")
        right_size = right_df.explain(mode="cost")
        # right_size = SizeEstimator.estimate(right_df)
